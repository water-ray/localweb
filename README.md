...
- `仓库：推送当前分支`：使用当前仓库默认远程源推送当前分支；遇到非 fast-forward 冲突时，先显示本地/远程提交说明差异，再输入 `Y` 或 `yes` 确认是否强推覆盖。
- `仓库：推送指定分支`：输入目标分支，使用当前仓库默认远程源将当前内容推送到指定远程分支；遇到冲突时同样支持二次确认强推。
- `仓库：拉取远程分支`：输入远程分支名，使用当前仓库默认远程源拉取；远程分支留空时复用 `temp/git_tasks/pull-remote.json` 中的该任务缓存。先自动提交本地改动，再检查远程分支，同名分支优先合并，合并失败或分支不同则确认后覆盖拉取。
...
- 最近使用的目标分支会按任务分别缓存到 `temp/git_tasks/<任务命令>.json`，任务之间不共用分支缓存；VS Code 的输入框不会动态显示这些缓存值，直接回车留空时由脚本读取对应任务缓存。
